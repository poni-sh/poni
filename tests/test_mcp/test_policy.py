...
class TestCliPolicy:
    ...
    def test_allows_valid_subcommand(self, policy: PolicyEngine):
        """Test that allowed subcommands pass."""
        result = policy.check_cli("aws", "s3 ls s3://bucket")
        assert result is None

    def test_blocks_denied_subcommand(self, policy: PolicyEngine):
    ...
    def test_blocks_unlisted_subcommand(self, policy: PolicyEngine):
    ...
    def test_blocks_deny_pattern(self, policy: PolicyEngine):
        """Test that deny patterns block commands."""
        result = policy.check_cli("aws", "s3 rm --recursive delete-bucket")
        assert result is not None
        assert "deny_patterns" in result

    def test_allow_patterns(self, policy: PolicyEngine):
    ...
    def test_require_patterns(self, policy: PolicyEngine):
...
class TestNamespacePolicy:
    ...
    def test_allows_valid_namespace(self, policy: PolicyEngine):
        """Test that allowed namespaces pass."""
        result = policy.check_cli("kubectl", "get pods -n staging")
        assert result is None

    def test_blocks_denied_namespace(self, policy: PolicyEngine):
    ...
    def test_blocks_unlisted_namespace(self, policy: PolicyEngine):
    ...
    def test_handles_long_namespace_flag(self, policy: PolicyEngine):
...
class TestInteractiveCheck:
    ...
    def test_detects_interactive_pattern(self, policy: PolicyEngine):
    ...
    def test_normal_command_not_interactive(self, policy: PolicyEngine):
        """Test that normal commands are not interactive."""
        assert policy.check_interactive("aws", "s3 ls") is False

    def test_unknown_cli_not_interactive(self, policy: PolicyEngine):
...
class TestRedaction:
    ...
    def test_redacts_patterns(self, policy: PolicyEngine):
    ...
    def test_preserves_other_content(self, policy: PolicyEngine):
    ...
    def test_unknown_cli_no_redaction(self, policy: PolicyEngine):
...
class TestTruncation:
    ...
    def test_truncates_long_output(self, policy: PolicyEngine):
    ...
    def test_preserves_short_output(self, policy: PolicyEngine):
        """Test that short output is not truncated."""
        output = "line1\nline2\nline3"
        result = policy.truncate_output("aws", output)
        assert result == output

    def test_no_truncation_without_config(self, policy: PolicyEngine):
